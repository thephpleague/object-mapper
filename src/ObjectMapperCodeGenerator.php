...
final class ObjectMapperCodeGenerator
{
    ...
    public function dump(array $classes, string $dumpedClassName): string
    {
        ...
        foreach ($hydrationClasses as $className) {
            ...
            $mapperMap[] = "'$className' => \$this->$methodName(\$payload),";
            $mappers[] = $this->dumpClassHydrator($className, $classDefinition);
        }

        $mapperMapCode = implode("\n                ", $mapperMap);
        $mapperCode = implode("\n\n", $mappers);
...
use League\ObjectMapper\UnableToHydrateObject;
use League\ObjectMapper\UnableToSerializeObject;
use Generator;
...
class $shortName implements ObjectMapper
{
    ...
    public function hydrateObject(string \$className, array \$payload): object
    {
        return match(\$className) {
            $mapperMapCode
            default => throw UnableToHydrateObject::noMapperDefined(\$className, \$this->hydrationStack),
        };
    }
